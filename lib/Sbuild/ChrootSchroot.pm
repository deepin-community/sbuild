...
sub begin_session {
	...
	my $schroot_session
	  = readpipe($self->get_conf('SCHROOT') . " -c $chroot --begin-session");
...
sub end_session {
	...
	print STDERR "Cleaning up chroot (session id "
	  ...
	  if $self->get_conf('DEBUG');
	system(
		$self->get_conf('SCHROOT'), '-c',
		$self->get('Session ID'),   '--end-session'
	);
	$self->set('Session ID', "");
	if ($?) {
...
sub get_command_internal {
	...
	if (defined($options->{'ENABLE_NETWORK'})
		...
		if ($self->get_conf('SCHROOT') =~ /unschroot/) {
			...
			print STDERR
"Disabling the network for this command was requested but the schroot backend doesn't support this feature yet: https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=802849\n"
			  if $self->get_conf('DEBUG');
		}
	}
