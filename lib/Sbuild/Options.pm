...
sub set_options {
	...
	my (
		$opt_apt_clean,       $opt_no_apt_clean,
		$opt_apt_update,      $opt_no_apt_update,
		$opt_apt_upgrade,     $opt_no_apt_upgrade,
		$opt_apt_distupgrade, $opt_no_apt_distupgrade
	);
	...
	my (
		$opt_make_binnmu,      $opt_binnmu,
		$opt_binnmu_timestamp, $opt_binnmu_changelog,
		$opt_append_to_version
	);
	...
	$self->add_options(
		"arch=s" => sub {
			...
			if (defined $opt_build_arch && $opt_build_arch ne $_[1]) {
				die
				  "cannot specify --arch together with differing --build-arch";
			}
			if (defined $opt_host_arch && $opt_host_arch ne $_[1]) {
				die
				  "cannot specify --arch together with differing --host-arch";
			}
		...
		"build=s" => sub {
			...
			if (defined $opt_arch && $opt_arch ne $_[1]) {
				die
				  "cannot specify --build-arch together with differing --arch";
			}
			$self->set_conf('BUILD_ARCH', $_[1]);
			$opt_build_arch = $_[1];
		},
		"host=s" => sub {
			...
			if (defined $opt_arch && $opt_arch ne $_[1]) {
				die
				  "cannot specify --host-arch together with differing --arch";
			}
			$self->set_conf('HOST_ARCH', $_[1]);
			$opt_host_arch = $_[1];
		},
		...
		"make-binNMU=s" => sub {
			if ($opt_binnmu_changelog) {
				die
"--make-binNMU cannot be used together with --binNMU-changelog";
			}
		...
		"binNMU-timestamp=s" => sub {
			if ($opt_binnmu_changelog) {
				die
"--binNMU-timestamp cannot be used together with --binNMU-changelog";
			}
		...
		"append-to-version=s" => sub {
			if ($opt_binnmu_changelog) {
				die
"--append-to-version cannot be used together with --binNMU-changelog";
			}
		...
		"binNMU-changelog=s" => sub {
			if ($opt_make_binnmu) {
				die
"--binNMU-changelog cannot be used together with --make-binNMU";
			}
			if ($opt_binnmu) {
			...
			if ($opt_binnmu_timestamp) {
				die
"--binNMU-changelog cannot be used together with --binNMU-timestamp";
			}
			if ($opt_append_to_version) {
				die
"--binNMU-changelog cannot be used together with --append-to-version";
			}
		...
		"autopkgtest-virt-server-opts=s" => sub {
			$self->push_conf('AUTOPKGTEST_VIRT_SERVER_OPTIONS',
				split(/\s+/, $_[1]));
		},
		...
		"apt-update" => sub {
			if ($opt_no_apt_update) {
				die
				  "--apt-update cannot be used together with --no-apt-update";
			}
		...
		"apt-upgrade" => sub {
			if ($opt_no_apt_upgrade) {
				die
"--apt-upgrade cannot be used together with --no-apt-upgrade";
			}
		...
		"apt-distupgrade" => sub {
			if ($opt_no_apt_distupgrade) {
				die
"--apt-distupgrade cannot be used together with --no-apt-distupgrade";
			}
		...
		"no-apt-update" => sub {
			if ($opt_apt_update) {
				die
				  "--no-apt-update cannot be used together with --apt-update";
			}
		...
		"no-apt-upgrade" => sub {
			if ($opt_apt_upgrade) {
				die
"--no-apt-upgrade cannot be used together with --apt-upgrade";
			}
		...
		"no-apt-distupgrade" => sub {
			if ($opt_apt_distupgrade) {
				die
"--no-apt-distupgrade cannot be used together with --apt-distupgrade";
			}
		...
		"debbuildopts=s" => sub {
			$self->push_conf('DPKG_BUILDPACKAGE_USER_OPTIONS',
				split(/\s+/, $_[1]));
		},
		"debbuildopt=s" => sub {
		...
		"hide-log-sections=s" => sub {
			$self->push_conf('LOG_HIDDEN_SECTIONS', split(/,/, $_[1]));
		},
		"log-filename-ts-format=s" => sub {
			$self->set_conf('LOG_FILENAME_TIMESTAMP_FORMAT', $_[1]);
		},
		"n|nolog" => sub {
		...
		"setup-hook=s" => sub {
			push(
				@{
					${ $self->get_conf('EXTERNAL_COMMANDS') }
		...
		"use-snapshot" => sub {
			...
			$self->set_conf('PATH',
				  '/usr/lib/gcc-snapshot/bin' . $self->get_conf('PATH') ne ''
				? ':' . $self->get_conf('PATH')
				: '');
		},
		...
		"resolve-alternatives" => sub {
			if ($opt_no_resolve_alternatives) {
				die
"--resolve-alternatives cannot be used together with --no-resolve-alternatives";
			}
		...
		"no-resolve-alternatives" => sub {
			if ($opt_resolve_alternatives) {
				die
"--no-resolve-alternatives cannot be used together with --resolve-alternatives";
			}
		...
		"clean-source" => sub {
			if ($opt_no_clean_source) {
				die
"--clean-source cannot be used together with --no-clean-source";
			}
		...
		"no-clean-source" => sub {
			if ($opt_clean_source) {
				die
"--no-clean-source cannot be used together with --clean-source";
			}
		...
		"run-lintian" => sub {
			if ($opt_no_run_lintian) {
				die
"--run-lintian cannot be used together with --no-run-lintian";
			}
		...
		"no-run-lintian" => sub {
			if ($opt_run_lintian) {
				die
"--no-run-lintian cannot be used together with --run-lintian";
			}
		...
		"run-piuparts" => sub {
			if ($opt_no_run_piuparts) {
				die
"--run-piuparts cannot be used together with --no-run-piuparts";
			}
		...
		"no-run-piuparts" => sub {
			if ($opt_run_piuparts) {
				die
"--no-run-piuparts cannot be used together with --run-piuparts";
			}
		...
		"run-autopkgtest" => sub {
			if ($opt_no_run_autopkgtest) {
				die
"--run-autopkgtest cannot be used together with --no-run-autopkgtest";
			}
		...
		"no-run-autopkgtest" => sub {
			if ($opt_run_autopkgtest) {
				die
"--no-run-autopkgtest cannot be used together with --run-autopkgtest";
			}
		...
		"enable-network" => sub {
			if ($opt_no_enable_network) {
				die
"--enable-network cannot be used together with --no-enable-network";
			}
		...
		"no-enable-network" => sub {
			if ($opt_enable_network) {
				die
"--no-enable-network cannot be used together with --enable-network";
			}
...
# This allows to pass -sa to all commands instead of passing it just to dpkg-buildpackage
	push(@{ $self->get_conf('DPKG_BUILDPACKAGE_USER_OPTIONS') }, "-sa")
	  if ( $self->get_conf('BUILD_SOURCE')
		&& $self->get_conf('FORCE_ORIG_SOURCE'));
}
