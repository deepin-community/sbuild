...
use Dpkg::Deps;
use Sbuild::Base;
use Sbuild qw($devnull isin debug debug2);
...
sub new {
	...
	$self->set(
		'Extra repositories archive list file',
		$extra_repositories_archive_list_file
	);

	my $extra_packages_archive_list_file
	  = '/etc/apt/sources.list.d/sbuild-extra-packages-archive.list';
	$self->set(
		'Extra packages archive list file',
		$extra_packages_archive_list_file
	);
...
sub add_extra_repositories {
	...
	if (scalar @{ $self->get_conf('EXTRA_REPOSITORIES') } > 0) {
		...
		{
			$self->log_error(
"$extra_repositories_archive_list_file exists - will not write extra repositories to it\n"
			);
		} else {
			...
			if (!$session->chmod($tmpfilename, '0644')) {
				$self->log(
					"Failed to create apt list file for dummy archive.\n");
				$session->unlink($tmpfilename);
				return 0;
			}
			if (
				!$session->rename(
					$tmpfilename, $extra_repositories_archive_list_file
				)
			) {
				$self->log(
					"Failed to create apt list file for dummy archive.\n");
...
# In that case, forbid installing host architecture binaries.
	if (grep { $_ eq 'cross' }
		(split /\s+/, $self->get_conf('BUILD_PROFILES'))) {
	...
	if (!$session->chown($aptconf, $self->get_conf('BUILD_USER'), 'sbuild')) {
		$self->log_error("Failed to set "
			  . $self->get_conf('BUILD_USER')
			  . ":sbuild ownership on apt.conf at $aptconf\n");
		return 0;
	}
	if (!$session->chmod($aptconf, '0664')) {
		$self->log_error(
			"Failed to set 0664 permissions on apt.conf at $aptconf\n");
		return 0;
	}
	...
	if (scalar @{ $self->get_conf('EXTRA_PACKAGES') } > 0) {
		...
		if ($session->test_regular_file($extra_packages_archive_list_file)) {
			$self->log_error(
"$extra_packages_archive_list_file exists - will not write extra packages archive list to it\n"
			);
		} else {
			#Prepare a path to place the extra packages
			if (!defined $self->get('Extra packages path')) {
				my $tmpdir = $session->mktemp({
						TEMPLATE => $self->get('Build Dir')
						  . '/resolver-XXXXXX',
						DIRECTORY => 1
					});
				if (!$tmpdir) {
					$self->log_error("mktemp -d "
						  . $self->get('Build Dir')
						  . '/resolver-XXXXXX failed\n');
					return 0;
				}
			...
			if (!$session->chmod($self->get('Extra packages path'), '0770')) {
				$self->log_error(
					"Failed to set 0770 permissions on extra packages dir\n");
				return 0;
			}
			my $extra_packages_dir = $self->get('Extra packages path');
			my $extra_packages_archive_dir
			  = $extra_packages_dir . '/apt_archive';
			my $extra_packages_release_file
			  = $extra_packages_archive_dir . '/Release';

			$self->set(
				'Extra packages archive directory',
				$extra_packages_archive_dir
			);
			$self->set('Extra packages release file',
				$extra_packages_release_file);
			my $extra_packages_archive_list_file
			  = $self->get('Extra packages archive list file');

			if (!$session->test_directory($extra_packages_dir)) {
				$self->log_warning(
						'Could not create build-depends extra packages dir '
					  . $extra_packages_dir . ': '
					  . $!);
				return 0;
			}
			if (
				!(
					$session->test_directory($extra_packages_archive_dir)
					|| $session->mkdir(
						$extra_packages_archive_dir, { MODE => "00775" }))
			) {
				$self->log_warning(
'Could not create build-depends extra packages archive dir '
					  . $extra_packages_archive_dir . ': '
					  . $!);
				return 0;
			}
			...
			for my $deb (@{ $self->get_conf('EXTRA_PACKAGES') }) {
				if (-f $deb) {
					...
					if (
						$session->test_regular_file(
							"$extra_packages_archive_dir/$base_deb")
					) {
						$self->log_warning(
"$base_deb already exists in $extra_packages_archive_dir inside the chroot. Skipping...\n"
						);
						next;
					}
					$self->log("Copying $deb to "
						  . $session->get('Location')
						  . "...\n");
					$session->copy_to_chroot($deb,
						$extra_packages_archive_dir);
				} elsif (-d $deb) {
					opendir(D, $deb);
					while (my $f = readdir(D)) {
						...
						if (
							$session->test_regular_file(
								"$extra_packages_archive_dir/$f")
						) {
							$self->log_warning(
"$f already exists in $extra_packages_archive_dir inside the chroot. Skipping...\n"
							);
							next;
						}
						$self->log("Copying $deb/$f to "
							  . $session->get('Location')
							  . "...\n");
						$session->copy_to_chroot("$deb/$f",
							$extra_packages_archive_dir);
					}
					closedir(D);
				} else {
...
"$deb is neither a regular file nor a directory. Skipping...\n"
			...
			if (
				!$self->run_apt_ftparchive(
					$self->get('Extra packages archive directory'))
			) {
	  ...
	  # Write a list file for the extra packages archive if one not create yet.
			if (
				...
				print $tmpfh 'deb [trusted=yes] file://'
				  . $extra_packages_archive_dir . " ./\n";
				print $tmpfh 'deb-src [trusted=yes] file://'
				  . $extra_packages_archive_dir . " ./\n";
				...
				if (!$session->chmod($tmpfilename, '0644')) {
					$self->log(
"Failed to create apt list file for extra packages archive.\n"
					);
					$session->unlink($tmpfilename);
					return 0;
				}
				if (
					!$session->rename(
						$tmpfilename, $extra_packages_archive_list_file
					)
				) {
					$self->log(
"Failed to create apt list file for extra packages archive.\n"
	...
	# build.
	#
	# Keys have to be in a format that apt supports in /etc/apt/trusted.gpg.d,
	# as they are just copied to there.
	if (@{ $self->get_conf('EXTRA_REPOSITORY_KEYS') }) {
		for my $repokey (@{ $self->get_conf('EXTRA_REPOSITORY_KEYS') }) {
			debug("Adding archive key: $repokey\n");
			if (!-f $repokey) {
				$self->log(
"Failed to add archive key '${repokey}' - it doesn't exist!\n"
				);
				return 0;
			}
			my $tmpfilename = mktemp("/tmp/tmp.XXXXXXXXXX");
			# Key might be armored but apt requires keys in binary format.
			# We manually convert the key as it is just base64 encoded data
			# with a header and footer, and then we do not need to invoke gpg.
			#
			# The decoding of armored gpg keys can even be done from a shell
			# script by using:
			#
			# awk '/^$/{ x = 1; } /^[^=-]/{ if (x) { print $0; } ; }' \
			#   | base64 -d
			#
			# As explained by dkg here:
			...
			if ($first_line eq "-----BEGIN PGP PUBLIC KEY BLOCK-----") {
				...
				print $out $first_line;    # initially read bytes
				while (my $line = <$fh>) {
					print $out $line;
				}
			}
			...
			# variable
			my $keyfilename = $session->mktemp({
				TEMPLATE =>
"/etc/apt/trusted.gpg.d/sbuild-extra-repository-XXXXXXXXXX.gpg"
			});
			if (!$keyfilename) {
				$self->log_error(
					"Can't create tempfile for external repository key\n");
			...
			if (!$session->copy_to_chroot($tmpfilename, $keyfilename)) {
				$self->log_error(
"Failed to copy external repository key $repokey into chroot $keyfilename\n"
				);
				$session->unlink($keyfilename);
				unlink $tmpfilename;
				return 0;
			...
			if (!$session->chmod($keyfilename, '0644')) {
				$self->log_error(
					"Failed to chmod $keyfilename inside the chroot\n");
				$session->unlink($keyfilename);
				return 0;
			}
		}
...
#Prepare a path to build a dummy package containing our deps:
	if (  !defined $self->get('Dummy package path')
		&& defined $self->get('Build Dir')) {
		my $tmpdir = $session->mktemp({
			TEMPLATE  => $self->get('Build Dir') . '/resolver-XXXXXX',
			DIRECTORY => 1
		});
		if (!$tmpdir) {
			$self->log_error("mktemp -d "
...
sub get_foreign_architectures {
	...
	$session->run_command({
		COMMAND => ['dpkg', '--assert-multi-arch'],
		USER    => 'root'
	});
	if ($?) {
	...
	my $foreignarchs = $session->read_command({
...
sub cleanup_foreign_architectures {
	...
	foreach my $arch (keys %{$added_foreign_arches}) {
		...
		if ($?) {
			$self->log_error(
				"Failed to remove dpkg foreign-architecture $arch\n");
			return;
		}
	}
}
...
sub update_archive {
	...
	if (!$self->get_conf('APT_UPDATE_ARCHIVE_ONLY')) {
		...
		my $dummy_sources_list_d
		  = $self->get('Dummy package path') . '/sources.list.d';
		if (
			!(
				   $session->test_directory($dummy_sources_list_d)
				|| $session->mkdir($dummy_sources_list_d, { MODE => "00700" }))
		) {
			$self->log_warning(
				'Could not create build-depends dummy sources.list directory '
				  . $dummy_sources_list_d . ': '
				  . $!);
			return 0;
		}
		...
		for my $list_file (
			$self->get('Dummy archive list file'),
			$self->get('Extra packages archive list file'),
			$self->get('Extra repositories archive list file')
		) {
...
sub upgrade {
	...
	$self->run_apt_command({
			COMMAND => [
				$self->get_conf('APT_GET'),
				'-uy', '-o', 'Dpkg::Options::=--force-confold', 'upgrade'
			],
			ENV  => { 'DEBIAN_FRONTEND' => 'noninteractive' },
			USER => 'root',
			DIR  => '/'
...
sub distupgrade {
	...
	$self->run_apt_command({
			COMMAND => [
				$self->get_conf('APT_GET'),
				'-uy', '-o', 'Dpkg::Options::=--force-confold',
				'dist-upgrade'
			],
			ENV  => { 'DEBIAN_FRONTEND' => 'noninteractive' },
			USER => 'root',
...
sub run_apt {
	...
	push @apt_command, '--allow-unauthenticated'
	  if ($self->get_conf('APT_ALLOW_UNAUTHENTICATED'));
	if ($self->get('Host Arch') ne $self->get('Build Arch')) {
		my $session = $self->get('Session');
...
sub run_xapt {
	...
	my $pipe         = $self->pipe_xapt_command({
...
sub get_dpkg_status {
	...
	my $STATUS
	  = $self->get('Session')->get_read_file_handle('/var/lib/dpkg/status');
	if (!$STATUS) {
	...
	while (<$STATUS>) {
		...
		if (!$pkg) {
			$self->log_error(
				"parse error in /var/lib/dpkg/status: no Package: field\n");
			next;
		}
		if (defined($version)) {
		...
		if (!$status) {
			$self->log_error(
"parse error in /var/lib/dpkg/status: no Status: field for package $pkg\n"
			);
			next;
		}
		...
		if (!defined $version || $version eq "") {
			$self->log_error(
"parse error in /var/lib/dpkg/status: no Version: field for package $pkg\n"
			);
			next;
		}
		...
		if ($provides) {
			foreach (split(/\s*,\s*/, $provides)) {
				$result{$_} = { Installed => 1, Version => '~*=PROVIDED=*=' }
				  if isin($_, @interest)
				  and (not exists($result{$_})
					or ($result{$_}->{'Installed'} == 0));
			}
		}
	}
	close($STATUS);
...
sub setup_apt_archive {
	...
	if (
		!$session->chown(
			$self->get('Dummy package path'), $self->get_conf('BUILD_USER'),
			'sbuild'
		)
	) {
		$self->log_error("Failed to set "
	...
	if (!$session->chmod($self->get('Dummy package path'), '0770')) {
		$self->log_error(
			"Failed to set 0770 permissions on dummy package dir\n");
		return 0;
	}
	...
	if (!$session->test_directory($dummy_dir)) {
		$self->log_warning('Could not create build-depends dummy dir '
			  . $dummy_dir . ': '
			  . $!);
		return 0;
	}
	if (
		!(
			   $session->test_directory($dummy_gpghome)
			|| $session->mkdir($dummy_gpghome, { MODE => "00700" }))
	) {
		$self->log_warning('Could not create build-depends dummy gpg home dir '
			  . $dummy_gpghome . ': '
			  . $!);
		return 0;
	}
	if (
		!$session->chown(
			$dummy_gpghome, $self->get_conf('BUILD_USER'), 'sbuild'
		)
	) {
		$self->log_error('Failed to set '
			  . $self->get_conf('BUILD_USER')
	...
	if (
		!(
			   $session->test_directory($dummy_archive_dir)
			|| $session->mkdir($dummy_archive_dir, { MODE => "00775" }))
	) {
		$self->log_warning('Could not create build-depends dummy archive dir '
			  . $dummy_archive_dir . ': '
			  . $!);
		return 0;
	}
	...
	if (!($session->mkdir("$dummy_pkg_dir", { MODE => "00775" }))) {
		$self->log_warning(
			'Could not create build-depends dummy dir ' . $dummy_pkg_dir . $!);
		return 0;
	}
	...
	my $DUMMY_CONTROL
	  = $session->get_write_file_handle("$dummy_pkg_dir/DEBIAN/control");
	if (!$DUMMY_CONTROL) {
		$self->log_warning('Could not open '
			  . $dummy_pkg_dir
			  . '/DEBIAN/control for writing: '
			  . $!);
		return 0;
	}
...
EOF
	...
	my $positive_build_deps
	  = join(", ", @positive, @positive_arch, @positive_indep);
	...
	if (!defined $positive) {
		my $msg
		  = "Error! deps_parse() couldn't parse the positive Build-Depends '$positive_build_deps'";
		$self->log_error("$msg\n");
		return 0;
	}

	my $negative_build_deps
	  = join(", ", @negative, @negative_arch, @negative_indep);
	...
	if (!defined $negative) {
		my $msg
		  = "Error! deps_parse() couldn't parse the negative Build-Depends '$negative_build_deps'";
		$self->log_error("$msg\n");
		return 0;
	}
...
EOF
	...
	foreach my $path (
		$dummy_pkg_dir . '/DEBIAN/control',
		$dummy_pkg_dir . '/DEBIAN',
		$dummy_pkg_dir, $dummy_archive_dir
	) {
...
# NO_PKG_MANGLE=1 disables https://launchpad.net/pkgbinarymangler (only used on Ubuntu)
	my @command = ('env', 'NO_PKG_MANGLE=1', 'dpkg-deb');

	# --root-owner-group was added in 1.19.0
	if ($self->get_dpkg_version() >= "1.19.0") {
		push(@command, '--root-owner-group');
	}

	push(@command, ('--build', $dummy_pkg_dir, $dummy_deb));
	...
	if (!$dummy_dsc_fh) {
		$self->log_warning(
			'Could not open ' . $dummy_dsc . ' for writing: ' . $!);
		return 0;
	}
...
EOF
	...
	if (scalar(@positive_arch)) {
		print $dummy_dsc_fh 'Build-Depends-Arch: '
		  . join(", ", @positive_arch) . "\n";
	}
	if (scalar(@negative_arch)) {
		print $dummy_dsc_fh 'Build-Conflicts-Arch: '
		  . join(", ", @negative_arch) . "\n";
	}
	if (scalar(@positive_indep)) {
		print $dummy_dsc_fh 'Build-Depends-Indep: '
		  . join(", ", @positive_indep) . "\n";
	}
	if (scalar(@negative_indep)) {
		print $dummy_dsc_fh 'Build-Conflicts-Indep: '
		  . join(", ", @negative_indep) . "\n";
	}
	print $dummy_dsc_fh "\n";
	close $dummy_dsc_fh;
	...
	if (!$session->test_regular_file($dummy_archive_list_file)) {
	   ...
	   # that apt feature is too new for it.
		print $tmpfh 'deb [trusted=yes] copy://'
		  . $dummy_archive_dir . " ./\n";
		print $tmpfh 'deb-src [trusted=yes] copy://'
		  . $dummy_archive_dir . " ./\n";
...
sub cleanup_apt_archive {
	...
	if (defined $self->get('Dummy package path')) {
		$session->unlink($self->get('Dummy package path'),
			{ RECURSIVE => 1, FORCE => 1 });
	}
	...
	$session->unlink($self->get('Extra repositories archive list file'),
		{ FORCE => 1 });

	$session->unlink($self->get('Extra packages archive list file'),
		{ FORCE => 1 });
