...
sub install_deps {
	...
	$self->log_subsubsection(
		"Install $name build dependencies (apt-based resolver)");
...
sub purge_extra_packages {
	...
	while (my $line = <$pipe>) {
		...
		if (   $1 eq "yes"
			|| $2 eq $dummy_pkg_name
			|| $2 eq $self->get_sbuild_dummy_pkg_name('core')) {
			push @essential, $2;
		} else {
			push @nonessential, $2;
		}
	...
	if (
		!$session->run_command(
			{ COMMAND => ['apt-mark', 'auto', @nonessential], USER => 'root' })
	) {
	...
	if (
		!$session->run_command(
			{ COMMAND => ['apt-mark', 'manual', @essential], USER => 'root' })
	) {
		$self->log_error("unable to run apt-mark\n");
		return 0;
	}
	...
	my $tmpfilename
	  = $session->mktemp({ USER => $self->get_conf('BUILD_USER') });
	if (!$tmpfilename) {
	...
	$self->run_apt(
		...
		\@rmvd,                            'autoremove',
		'apt',                             '-o',
		'Dir::State::Lists=/dev/null',     '-o',
		"Dir::State::Status=$tmpfilename", '--allow-remove-essential'
	);
